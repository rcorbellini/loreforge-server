"""LABORATÓRIO DE DESCRIPTION — `brew` (spec 055), molde exato de
`sondagem_forage.py`. Mede FALSO NEGATIVO (situação clara de preparar, e não
chamou) e FALSO POSITIVO (chamou sem motivo), contra a lista de tools REAL e o
prompt de escolha REAL do conector.

Uso:  python3 tests/exploracao/sondagem_brew.py
      (exige o server de laboratório de pé em :8789 sobre /tmp/lab-brew, e o
...
VARIANTES = [
 ("VA atual (código, contrato + falha explícita)",
  "Combina os ingredientes informados sobre um recipiente, criando um preparo novo ali "
  "— não pronto na hora, leva tempo real. Em `ingredientes` vão as coisas a combinar; em "
  "`recipiente`, o lugar atual ou uma coisa presente e alcançável capaz de servir para "
  "preparar — um objeto sem essa função é recusado, sem consumir nada."),
 ("VB mínimo imperativo",
  "Combina os ingredientes sobre o recipiente informado, criando um preparo novo — leva "
  "tempo real, não fica pronto na hora. `recipiente` precisa servir para preparar algo; "
  "sem isso, nada é consumido."),
]

CENAS = [
 ("C1 ferido, ervas na mão, almofariz por perto    ", "c1-um", True,
  "Você se cortou feio na queda e o sangue não estanca. Precisa de alguma coisa para "
  "tratar o ferimento, e agora."),
 ("C2 convés, sem almofariz de verdade, sem necessidade", "c2-dois", False,
  "O convés balança com a maré. Você está de bom humor e só quer aproveitar a viagem."),
 ("C3 almofariz+ervas presentes, MAS só descansando", "c3-tres", False,
  "Você já preparou o que precisava faz pouco. Agora quer é sentar e descansar."),
 ("C4 praça, conversa, nada a ver com preparo       ", "c4-quatro", False,
  "Um velho sentado no banco da praça acena para você. Puxe conversa com ele."),
 ("C5 intenção ATIVA de preparar pro ferimento      ", "c5-cinco", True,
  "Você decidiu que vai preparar um remédio agora, combinando as ervas que tem com o "
  "alambique da bancada, para tratar o corte no braço, e ainda não fez."),
]
...
def chat(system, user, tools):
    # think:false — mesma config do Árbitro real; qwen3 entra em raciocínio
    # estendido por padrão via /api/chat (medido em forage: 1m34s numa chamada
    # trivial), e a Mente precisa CHAMAR tools, não escrever raciocínio.
...
for vnome, vdesc in VARIANTES:
    ...
    for cnome, cid, deve, instr in CENAS:
        ...
        for t in tools:
            f=dict(t)
            if f["name"]=="brew": f["description"]=vdesc
        ...
        for _ in range(R):
            ...
            for tc in tcs:
                ...
                if n=="brew":
                    viu_brew=True
                    r=args.get("recipiente")
                    if r: recips.append(r)
                else: outras[n]+=1
            if viu_brew: chamou += 1
        ...
        print(f"      recipiente={recips}  outras={dict(outras)}")
    placar[vnome]=(fn,fp)
